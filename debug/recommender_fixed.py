...
ORIGINAL BUGGY CODE (lines 51-55):
...
TWO BUGS (both on lines 54-55):

BUG 1 (line 54): `profile_norm = np.linalg.norm(cohort_baseline)`
  - Computes the norm of the COHORT BASELINE instead of the student_profile.
  - Should be: `profile_norm = np.linalg.norm(student_profile)`

BUG 2 (line 55): `student_profile = cohort_baseline / (profile_norm + 1e-10)`
  - Completely OVERWRITES student_profile with the normalized cohort_baseline.
  - This means EVERY student gets the same profile (the cohort average),
    making all recommendations identical regardless of individual weaknesses.
  - Should be: `student_profile = student_profile / (profile_norm + 1e-10)`

ROOT CAUSE: The variable names were swapped — `cohort_baseline` was used where
`student_profile` should have been, in both the norm calculation and the
normalization step. This is a subtle copy-paste style bug that produces no
runtime errors. The code runs fine, but every student receives the exact same
recommendations because their individual profile is discarded and replaced
with the cohort average.

EFFECT: All 3 students (Arjun, Priya, Rahul) get identical recommendations
even though they have completely different weakness profiles (Physics, Chemistry,
and Math respectively). The overlap analysis shows 10/10 common questions
between all pairs.

FIXED CODE (lines 54-55):
...
def recommend(student_matrix: np.ndarray, question_matrix: np.ndarray,
    ...
    # === FIX: Use student_profile (not cohort_baseline) for both norm and normalization ===
    profile_norm = np.linalg.norm(student_profile)                # FIXED (was: cohort_baseline)
    student_profile = student_profile / (profile_norm + 1e-10)    # FIXED (was: cohort_baseline / ...)
